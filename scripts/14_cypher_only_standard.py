"""Additional ablation: Cypher-only retrieval with standard triples.

This standalone script keeps Cypher analysis and graph retrieval identical to
the Cypher-only attribute-enhanced experiment, but removes all node and
relation attributes before constructing the answer context and before saving
retrieved evidence.  The answer model therefore receives only
head-relation-tail triples.

Set LLM_API_KEY and NEO4J_PASSWORD in the environment before running.
"""
...
CONDITION_NAME = "cypher_standard"
...
def standardize(rows: List[Dict[str, Any]]) -> List[Dict[str, str]]:
    """Project retrieved records to strict head-relation-tail triples."""
    return [
        {
            ...
            "relation": str(
                ...
                or row.get("relation_label")
                or "unknown"
            ),
            "tail": str(row.get("tail", "unknown")),
        }
        for row in rows
    ]
...
def build_context(rows: List[Dict[str, str]]) -> str:
    ...
    return "Retrieved knowledge:\n" + "\n".join(
        f"{i}. {row['head']} -> [{row['relation']}] -> {row['tail']}"
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Cypher-only standard-triple ablation")
    ...
    try:
        for i, item in enumerate(questions, 1):
            ...
            info = cache[q]
            raw_rows, status = graph.cypher_search(info.get("cypher_condition", ""))
...
Return one answer letter (A, B, C, or D) on the first line, followed by a concise explanation.""")
            ...
            records.append({**item, "result": {"answer": pred, "reasoning": raw, "is_correct": ok,
                "retrieval": "cypher_only", "attribute_enhanced": False,
                "cypher_condition": info.get("cypher_condition", ""), "cypher_status": status,
                "cypher_count": len(raw_rows), "rows": rows}})
    ...
    summary = {"condition": CONDITION_NAME, "total_questions": len(records), "correct_answers": correct,
               ...
               "model_name": LLM_MODEL, "retrieval": "cypher_only", "attribute_enhanced_context": False}
